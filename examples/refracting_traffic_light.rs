...
pub fn traffic_light() -> (World, Camera) {
    ...
    let white_material = Arc::new(Lambertian::new(Color::new(0.9, 0.9, 0.9)));
    ...
    let others = 0.01;
    let red_light = Arc::new(DiffuseLight::new(
        Color::new(1.0, others, others) * light_intensity,
    ));
    let green_light = Arc::new(DiffuseLight::new(
        Color::new(others, 1.0, others) * light_intensity,
    ));
    let blue_light = Arc::new(DiffuseLight::new(
        Color::new(others, others, 1.0) * light_intensity,
    ));
