...
pub fn spining_balls() -> (World, Camera) {
    ...
    let ground_material: Arc<dyn Material> = Lambertian::new(Color::new(0.39, 0.26, 0.13));
    ...
    let fuzz: f64 = 0.7;
    let glass: Arc<dyn Material> = Dielectric::new(1.5); //try change to Lambertian
    let green: Arc<dyn Material> = Metal::with_fuzz(Color::new(0.1, 0.60, 0.1), fuzz);
    let red: Arc<dyn Material> = Metal::with_fuzz(Color::new(0.60, 0.1, 0.1), fuzz);
    let blue: Arc<dyn Material> = Metal::with_fuzz(Color::new(0.1, 0.1, 0.60), fuzz);
    let difflight: Arc<dyn Material> =
        DiffuseLight::new(Color::new(0.73, 0.73, 0.73) * light_intensity);
